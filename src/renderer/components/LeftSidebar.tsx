          ...
          <div className="p-4 flex flex-col h-full relative z-10">
           ...
           <div className="flex-grow flex flex-col justify-center items-center relative perspective-[1000px]">
               ...
               <div className="absolute w-32 h-48 border border-slate-700/30 rounded-3xl -z-10 flex items-center justify-center">
                   <div className="w-full h-full bg-slate-800/20 backdrop-blur-[1px] rounded-3xl"></div>
               </div>
               ...
               <div className="w-full h-64 relative max-w-[240px]">
                   ...
                   <div className="absolute top-[15%] left-4 right-4 h-[1px] bg-gradient-to-r from-transparent via-slate-600/30 to-transparent flex items-center justify-center">
                        ...
                        <button
                          disabled={disabled}
                          onClick={onLinkFront}
                          className={`w-6 h-6 rounded-full bg-slate-900 border border-slate-700 flex items-center justify-center transition-all z-20 group ${disabled ? 'opacity-50 cursor-not-allowed' : 'hover:scale-110 hover:border-indigo-400 hover:shadow-[0_0_10px_rgba(99,102,241,0.4)]'}`}
                        >
                            <span className="material-icons text-[10px] text-slate-500 group-hover:text-indigo-400 transition-colors">link</span>
                        </button>
                   </div>
                   ...
                   <div className="absolute bottom-[15%] left-4 right-4 h-[1px] bg-gradient-to-r from-transparent via-slate-600/30 to-transparent flex items-center justify-center">
                    ...
                    >
                        <div className={`w-full h-full bg-slate-800 border rounded-lg relative overflow-hidden shadow-lg transition-all ${
                          selectedWheels.FL ? 'border-blue-500/60 shadow-blue-500/20' : 'border-slate-600/50 group-hover:shadow-blue-500/20 group-hover:border-blue-500/50'
                        }`}>
                            <div className="absolute inset-0 flex flex-col justify-between py-1 opacity-20">
                                {[...Array(6)].map((_, i) => <div key={i} className="h-[2px] bg-slate-400 w-full"></div>)}
                            </div>
                            <div className="absolute inset-0 flex items-center justify-center">
                                <span className={`text-[10px] font-bold font-display ${selectedWheels.FL ? 'text-blue-300' : 'text-slate-400 group-hover:text-blue-400'}`}>FL</span>
                            </div>
                            {/* Status Dot */}
                            <div className={`absolute top-1 right-1 w-1.5 h-1.5 rounded-full ${selectedWheels.FL ? 'bg-blue-500 shadow-[0_0_5px_#3b82f6]' : 'bg-emerald-500 shadow-[0_0_5px_#10b981]'}`}></div>
                        </div>
                    </button>
                    ...
                    >
                        <div className={`w-full h-full bg-slate-800 border rounded-lg relative overflow-hidden shadow-lg transition-all ${
                          selectedWheels.FR ? 'border-blue-500/60 shadow-blue-500/20' : 'border-slate-600/50 group-hover:shadow-blue-500/20 group-hover:border-blue-500/50'
                        }`}>
                             <div className="absolute inset-0 flex flex-col justify-between py-1 opacity-20">
                                {[...Array(6)].map((_, i) => <div key={i} className="h-[2px] bg-slate-400 w-full"></div>)}
                            </div>
                            <div className="absolute inset-0 flex items-center justify-center">
                                <span className={`text-[10px] font-bold font-display ${selectedWheels.FR ? 'text-blue-300' : 'text-slate-400 group-hover:text-blue-400'}`}>FR</span>
                            </div>
                            <div className={`absolute top-1 left-1 w-1.5 h-1.5 rounded-full ${selectedWheels.FR ? 'bg-blue-500 shadow-[0_0_5px_#3b82f6]' : 'bg-emerald-500 shadow-[0_0_5px_#10b981]'}`}></div>
                        </div>
                    </button>
                    ...
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                        <div className="relative w-24 h-24 flex items-center justify-center">
                            {/* Rotating Rings */}
                            <div className="absolute inset-0 border border-slate-700/50 rounded-full animate-[spin_8s_linear_infinite]"></div>
                            <div className="absolute inset-2 border border-slate-700/30 rounded-full animate-[spin_12s_linear_infinite_reverse]"></div>
                            
                            {/* Core Button - Restore CENTER text and Red Color */}
                            <button className="w-14 h-14 rounded-full bg-slate-900 border border-red-500/30 flex items-center justify-center shadow-[0_0_15px_rgba(239,68,68,0.2)] group hover:scale-105 hover:bg-red-500/10 hover:border-red-500 hover:shadow-[0_0_20px_rgba(239,68,68,0.5)] transition-all z-10">
                                <span className="text-[10px] font-bold text-red-500 group-hover:text-white transition-colors tracking-widest">CENTER</span>
                            </button>
                            
                            {/* Connecting Beams */}
                            <div className="absolute w-full h-[1px] bg-slate-700/20 rotate-45"></div>
                            <div className="absolute w-full h-[1px] bg-slate-700/20 -rotate-45"></div>
                        </div>
                    </div>
                    ...
                    >
                        <div className={`w-full h-full bg-slate-800 border rounded-lg relative overflow-hidden shadow-lg transition-all ${
                          selectedWheels.RL ? 'border-blue-500/60 shadow-blue-500/20' : 'border-slate-600/50 group-hover:shadow-blue-500/20 group-hover:border-blue-500/50'
                        }`}>
                             <div className="absolute inset-0 flex flex-col justify-between py-1 opacity-20">
                                {[...Array(6)].map((_, i) => <div key={i} className="h-[2px] bg-slate-400 w-full"></div>)}
                            </div>
                            <div className="absolute inset-0 flex items-center justify-center">
                                <span className={`text-[10px] font-bold font-display ${selectedWheels.RL ? 'text-blue-300' : 'text-slate-400 group-hover:text-blue-400'}`}>RL</span>
                            </div>
                            <div className={`absolute bottom-1 right-1 w-1.5 h-1.5 rounded-full ${selectedWheels.RL ? 'bg-blue-500 shadow-[0_0_5px_#3b82f6]' : 'bg-emerald-500 shadow-[0_0_5px_#10b981]'}`}></div>
                        </div>
                    </button>
                    ...
                    >
                        <div className={`w-full h-full bg-slate-800 border rounded-lg relative overflow-hidden shadow-lg transition-all ${
                          selectedWheels.RR ? 'border-blue-500/60 shadow-blue-500/20' : 'border-slate-600/50 group-hover:shadow-blue-500/20 group-hover:border-blue-500/50'
                        }`}>
                             <div className="absolute inset-0 flex flex-col justify-between py-1 opacity-20">
                                {[...Array(6)].map((_, i) => <div key={i} className="h-[2px] bg-slate-400 w-full"></div>)}
                            </div>
                            <div className="absolute inset-0 flex items-center justify-center">
                                <span className={`text-[10px] font-bold font-display ${selectedWheels.RR ? 'text-blue-300' : 'text-slate-400 group-hover:text-blue-400'}`}>RR</span>
                            </div>
                            <div className={`absolute bottom-1 left-1 w-1.5 h-1.5 rounded-full ${selectedWheels.RR ? 'bg-blue-500 shadow-[0_0_5px_#3b82f6]' : 'bg-emerald-500 shadow-[0_0_5px_#10b981]'}`}></div>
                        </div>
                    </button>
